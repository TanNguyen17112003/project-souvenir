...
function ProductInCart({id, name, link, number, cost}) {
    ...
    const updateProductNumber = () => {
        axios.post("http://localhost:3400/cart/number", { numberOfProduct: numberOfProduct, email: userEmail, idProduct: id })
            .then((response) => {
    ...
    const handleRemoveProduct = async () => {
        try {
            await axios.delete(`http://localhost:3400/cart/remove/${userEmail}/${id}`)
